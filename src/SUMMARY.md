# Summary

# Introduction

- [Foreword]()
- [License](./introduction/license.md)

# Fundamentals

- [Variable types](./fundamentals/variable_types.md)
- [Time](./fundamentals/time.md)
- [Geometry](./fundamentals/geometry.md)
- [Coordinate systems](./fundamentals/coordinate_systems.md)

# Coordinate Transformations
- [Introduction](./transformations/motivation.md)
- [Transformation tree](./transformations/transformation-tree.md)
- [Transformation matrix](./transformations/transformation-matrix.md)
- [Rotation](./transformations/rotation.md)
- [Transformations over time](./transformations/time.md)
- [Implementation in Python](./transformations/implementation.md)

# Data Handling

- [Data representation](./data-preprocessing/data-representation.md)
- [Interpolation](./data-preprocessing/interpolation.md)
- [Merging datasets](./data-preprocessing/merging.md)

# Perception

- [Overview](perception/overview.md)
- [RADAR](perception/radar.md)
- [LIDAR](perception/lidar.md)
- [Sensor Fusion](perception/sensor_fusion.md)
- [Fusion Architectures](perception/architectures.md)

# Maps and Geography

- [Fundamentals](maps/fundamentals.md)
- [GPS](maps/gps.md)
- [Projections](maps/projections.md)
- [Spatial Indexing](maps/spatial_indexing.md)
- [Open Street Maps](maps/open-street-maps.md)


# In-vehicle networks

- [Overview](in-vehicle-networks/intro.md)
- [Controller Area Network](in-vehicle-networks/can_bus.md)
